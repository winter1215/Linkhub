...
import com.linkhub.portal.im.handler.MessageHandler;
...
@Component
public class WordWebSocketHandler extends TextWebSocketHandler implements InitializingBean {
    ...
    public void handleTextMessage(WebSocketSession session, TextMessage textMessage) throws Exception {
        ...
        try {
            ...
            // 通过 handler 获取其处理的消息 class(也可以直接根据 type 取)
            Class<? extends Message> messageClass = this.getMessageClass(messageHandler);
            // 处理消息
            Message messageObj = JSON.parseObject(jsonMessage.getString("body"), messageClass);
            //messageHandler.execute(session, messageObj);
        } catch (Throwable throwable) {
            logger.warn("[onMessage][session({}) message({}) 发生异常]", session, throwable);
        }
    }
    ...
    /**
     * 通过 handler 获取被该 handler 处理的 requestType || 也可以不使用这种方法获取 class,直接在 Message 中维护一个 map
     * @param handler: handler 会被 MessageHandler<XXXRequest> 实现,要获取对应的 requestType 就需要取到顶级父类的泛型参数
     * @author: winter
     * @description:
     */
